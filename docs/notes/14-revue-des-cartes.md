...
| `copper_edge_clearance`, `hole_near_hole` via contre via du même net | Rangée de pastilles sud du module ESP32 à moins de 0,5 mm du bord ; vias du routeur posées à 0,3 mm de la via d'éventail du même net (pastilles qui se recouvrent, trous trop proches). | Module remonté de 1,5 mm ; une via n'est plus émise dès qu'une via du même net la recouvre. |
| `copper_edge_clearance` sur des vias d'éventail | Les éventails des broches sud du module ESP32 posaient leur via à 0,2 mm du bord. | Le bord de carte (moins le dégagement) est un obstacle pour les éventails : la via recule d'une rangée ou disparaît, le routeur part du tronçon. |
