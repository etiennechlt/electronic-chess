...
class Circuit:
    ...
    def add(
        ...
        if any(c.ref == ref for c in self.components):
            raise ValueError(f"{ref}: duplicate reference")
        ...
        comp = Component(
            ref=ref, part=part, value=value, pins=dict(pins), nc=tuple(nc), dnp=dnp, sym=sym
        )
...
FB = Part(
    "Device", "FerriteBead", "Resistor_SMD:R_0805_2012Metric", mpn="BLM21PG221SN1D", lcsc="C18305"
)
LBUCK = Part("Device", "L", "Inductor_SMD:L_Bourns-SRN4018", mpn="SRN4018TA2R2M", lcsc="C2827538")
...
MUX = Part("4xxx", "4052", "Package_SO:TSSOP-16_4.4x5mm_P0.65mm", mpn="74HCT4052PW", lcsc="C57078")
INA = Part(
...
NFET = Part("Transistor_FET", "AO3400A", "Package_TO_SOT_SMD:SOT-23", mpn="AO3400A", lcsc="C20917")
PFET = Part("Transistor_FET", "AO3401A", "Package_TO_SOT_SMD:SOT-23", mpn="AO3401A", lcsc="C15127")
...
H1X12 = Part(
    "Connector_Generic", "Conn_01x12", "Connector_PinHeader_2.54mm:PinHeader_1x12_P2.54mm_Vertical"
)
H2X10 = Part(
    "Connector_Generic",
    "Conn_02x10_Odd_Even",
...
def build_circuit(cfg: BoardConfig) -> tuple[Circuit, ChainDesign]:
    ...
    for k in range(1, 5):
        ...
        ckt.add(f"Q{k + 20}", PFET, "AO3401A", {"1": f"DAMP{k}_N", "2": a, "3": f"DMP{k}"})
        r(
            f"R{base + 6}",
            str(int(drv.damp_r_ohm)) + "R",
            f"DMP{k}",
            b,
    ...
    ckt.add("J2", H1X12, "COIL JOINT", {str(i + 1): net for i, net in enumerate(JOINT_ORDER)})
    ckt.add(
        "J4",
        H2X10,
        "MCU",
    ...
    ckt.add(
        ...
        BUF5,
        "74AHCT1G125",
        {"1": GND, "2": "LED_DIN", "3": GND, "4": "LED_DINB", "5": "5V_BUCK"},
    )
