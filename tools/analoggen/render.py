...
def render_pcb(
    result: PcbResult, circuit: Circuit, out_path: Path, dpi: int = 220, show_plane: bool = False
) -> None:
    ...
    if show_plane:
        for poly in result.plane_polys:
            ax.add_patch(plt.Polygon(poly, closed=True, color="#1d3a52", zorder=1, linewidth=0))

    pads = _pad_instances(circuit, result.placements)
    for p in pads:
        color = "#d8d8a8" if p.tht else "#a07878"
        ax.add_patch(plt.Rectangle((p.x - p.w / 2, p.y - p.h / 2), p.w, p.h, color=color, zorder=4))
    ...
    for ref, (x, y, _rot) in result.placements.items():
        if ref in majors:
            ax.annotate(ref, (x, y - 2.2), color="#9ab0c4", fontsize=8, ha="center", zorder=7)

    ax.plot(
        [0, BOARD_W, BOARD_W, 0, 0],
        [0, 0, BOARD_H, BOARD_H, 0],
        color="#e6c832",
        linewidth=1.4,
        zorder=8,
    )
