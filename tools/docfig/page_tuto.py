...
<tr><td>Quadrant 2 x 2</td><td>généré, routage fermé, DRC KiCad : zéro erreur, zéro élément non connecté</td><td>rien : les fichiers de fabrication sont dans le dépôt</td></tr>
...
<tr><td>Devis de fabrication</td><td>à faire ; les options et la décision du cuivre interne sont arrêtées (note 23 du dépôt)</td><td>même session</td></tr>
<tr><td>Un point de fiche technique</td><td>l'aiguilleur ADG1607 alimenté en 5 V ; les vias de 0,45 mm sont tranchés, ils passent dans le procédé standard en quatre couches</td><td>lecture de la fiche</td></tr>
</tbody>
</table>
</div>
<p>Les deux cartes sont à zéro élément non connecté et zéro erreur bloquante au DRC (<code>tools/drc.py</code> dans le dépôt), et leurs fichiers de fabrication portent l'empreinte de la carte dont ils sortent : elles se commandent telles quelles.</p>
</section>
...
<tr><td>Quadrant 2 x 2</td><td>{{QUAD}} mm, 1,6 mm</td><td class="n">4</td><td>ses vias de 0,45 mm à perçage 0,2 mm passent dans le procédé standard en quatre couches, sans option ni surcoût ; prendre le cuivre interne 1 oz (voir ci dessous) ; un pochoir rend la pose du frontal beaucoup plus sûre</td></tr>
</tbody>
</table>
</div>
<p>Cinq exemplaires sont le minimum chez JLCPCB et suffisent largement. Le dépôt versionne les gerbers des deux cartes, il n'y a rien à produire : <code>hardware/bench/bench-gerbers.zip</code> et <code>hardware/quadrant-2x2/quadrant-2x2-gerbers.zip</code> se déposent tels quels sur le site du fabricant, après vérification de leur empreinte.

<p>Une option se décide ici, le cuivre interne du quadrant : l'empilement standard en quatre couches donne 1 oz dehors et 0,5 oz dedans, alors que la spirale de détection est calculée avec 35 µm sur ses quatre couches. En 0,5 oz interne, sa résistance monte de 50 % et son facteur Q descend d'un tiers, de 67 à 45 à 400 kHz. Pour une carte dont le métier est de mesurer, prendre l'option 1 oz interne.</p>

<p> Ils sortent de <code>tools/gerbers.py</code>, qui remplit les pours de cuivre avant de tracer : un plan de masse exporté sans ce remplissage arrive vide chez le fabricant. Les fichiers <code>jlc-bom.csv</code> et <code>jlc-cpl.csv</code> de chaque carte servent si l'on choisit l'assemblage en usine des composants qui ont un code ; les autres se soudent à la main.</p>
