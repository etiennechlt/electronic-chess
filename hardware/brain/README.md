...
- **MCU** : STM32G474RE soudé (LQFP-64), sans quartz (HSI plus CRS sur
  ...
  USB-C en périphérique (USBLC6-2 en protection, 5,1 k sur CC). Le
  tableau des broches est `plateau.brain.mcu_pins` dans le yaml, à
  valider contre la fiche technique avant fabrication (quatre entrées
  ADC sur quatre convertisseurs distincts, PB8 sert de BOOT0, PG10 de
  NRST).
...
- **Communication** : UART isolée ADuM1201, load switch P-FET sur le
  5 V du module, régulateur AMS1117 pour l'ESP32-S3-WROOM-1, cavalier
  JP1 pour alimenter le côté isolé depuis l'ESP32 ou le Pi, embase
  Pi 2 x 4, embase de programmation ESP 1 x 6, boutons BOOT et EN.
