/* Module interfaces of the brain board firmware (four 4x4 quadrants). */
...
/* adc.c: one converter per quadrant, one-shot burst of ADC_SAMPLES via DMA. */
void adc_init(void);
...
/* comp.c: COMP3 (PA0, quadrant 1) against DAC3 threshold, captured by TIM2 CH4. */
void comp_init(void);
...
void led_init(void);
void led_set_square(uint32_t square, led_camp_t camp); /* 0..63, file + 8 * rank */
void led_apply(void);
